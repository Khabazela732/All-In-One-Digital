...
class HostComapany(models.Model):
    ...
    passcode = models.CharField(
        max_length=50, default="internship#@", blank=True, null=True
    )
    contant_number = models.CharField(
...
class Rotation(models.Model):
    intern = models.ForeignKey("administration.IntershipEnrollment", on_delete=models.CASCADE, related_name="rotations")
    ...
    def __str__(self):
        return f"{self.intern.get_full_name()} - {self.department.name} ({self.start_date} to {self.end_date})"
